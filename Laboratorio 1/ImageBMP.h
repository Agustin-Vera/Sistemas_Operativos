...
typedef struct {
    ...
    uint16_t planes; // Planos, tiene 2 bytes y es un numero que indica la cantidad de planos de color, debe valer 1
    uint16_t bit_count; // Bits por pixel, tiene 2 bytes y es un numero que indica la cantidad de bits por pixel, debe valer 1, 4, 8, 16, 24, or 32
    uint32_t compression; // Compresion, tiene 4 bytes y es un numero que indica el tipo de compresion, puede ser 0 (sin compresion), 1 (RLE 8-bit/pixel), 2 (RLE 4-bit/pixel), 3 (bitfields), 4 (JPEG), 5 (PNG)
